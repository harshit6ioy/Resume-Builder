  ...
  return (
    <div className="w-full bg-slate-900 text-slate-300 h-[842px] border border-slate-700 aspect-[1/1.414] font-sans flex flex-col overflow-hidden shadow-sm" style={rootStyle}>
      <div className="p-8 pb-6 border-b border-slate-800 flex justify-between items-end">
        ...
        <div className="flex flex-col items-end text-sm text-slate-400" style={textStyle}>
      ...
      <div className="p-8 flex-1 grid grid-cols-1 gap-6">
        {data.summary && (
          <div className="bg-slate-800/50 p-6 rounded-xl border border-slate-700">
             <p className="text-sm leading-relaxed text-slate-300" style={textStyle}>{data.summary}</p>
          </div>
        )}
             ...
             {data.skills && data.skills.length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(Array.isArray(data.skills) ? data.skills : (data.skills ? data.skills.split(',') : [])).map((skill, index) => (
                      <span key={index} className="text-xs font-medium bg-emerald-900/30 text-emerald-300 px-3 py-1.5 rounded-md border border-emerald-800/50" style={textStyle}>
                        {skill}
                      </span>
                    ))}
                  </div>
              ...
              {data.education && (
                <div>
                  ...
                  <div className="text-sm whitespace-pre-wrap text-slate-400 leading-relaxed" style={textStyle}>{data.education}</div>
                </div>
              )}
              {data.activity_details && (
                <div>
                  ...
                  <div className="text-sm whitespace-pre-wrap text-slate-400 leading-relaxed" style={textStyle}>{data.activity_details}</div>
                </div>
              )}
           </div>
           ...
           <div className="col-span-2 space-y-6 border-l border-slate-800 pl-8">
             {data.experience && (
                <div>
                  ...
                  <div className="text-sm whitespace-pre-wrap leading-relaxed text-slate-300" style={textStyle}>{data.experience}</div>
                </div>
              )}
              {data.projects && (
                <div>
                  ...
                  <div className="text-sm whitespace-pre-wrap leading-relaxed text-slate-300" style={textStyle}>{data.projects}</div>
                </div>
              )}
           </div>
        </div>
