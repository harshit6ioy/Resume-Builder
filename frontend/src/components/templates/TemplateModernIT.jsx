    ...
    <div className="w-full bg-white text-slate-800 h-[842px] border border-slate-200 aspect-[1/1.414] font-sans flex flex-col overflow-hidden shadow-sm" style={rootStyle}>
      <div className="grid grid-cols-3 h-full">
        ...
        <div className="col-span-1 bg-slate-900 text-slate-300 p-8 flex flex-col">
          <div className="mb-10">
            <h1 className="text-3xl font-bold text-white mb-2 leading-tight" style={getStyle('full_name')}>{data.full_name || 'Your Name'}</h1>
            <div className="w-12 h-1 bg-cyan-500 mb-6"></div>
            <div className="space-y-2 text-xs text-slate-400 font-medium">
              {data.email && <div className="flex items-center gap-2" style={getStyle('email')}><svg className="w-4 h-4 text-cyan-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>{data.email}</div>}
              {data.phone && <div className="flex items-center gap-2" style={getStyle('phone')}><svg className="w-4 h-4 text-cyan-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>{data.phone}</div>}
            </div>
          </div>
            ...
            <div className="mb-8">
              ...
              <div className="flex flex-col gap-2">
                {(Array.isArray(data.skills) ? data.skills : (data.skills ? data.skills.split(',') : [])).map((skill, index) => (
                  <span key={index} className="text-xs bg-slate-800 text-cyan-100 px-3 py-1.5 rounded border border-slate-700/50" style={getStyle('skills')}>
                    {skill}
                  </span>
                ))}
              </div>
          ...
          {data.education && (
            <div>
              ...
              <div className="text-xs whitespace-pre-wrap leading-relaxed text-slate-400" style={getStyle('education')}>{data.education}</div>
            </div>
          )}
          {data.activity_details && (
            <div className="mt-8">
              ...
              <div className="text-xs whitespace-pre-wrap leading-relaxed text-slate-400" style={getStyle('activity_details')}>{data.activity_details}</div>
            </div>
          )}
        </div>
        ...
        <div className="col-span-2 p-10 bg-slate-50 flex flex-col gap-10">
          ...
          {data.summary && (
            <div>
              ...
              <p className="text-sm leading-relaxed text-slate-600 border-l-2 border-cyan-200 pl-4" style={getStyle('summary')}>{data.summary}</p>
            </div>
          )}
          ...
          {data.experience && (
            <div>
              ...
              <div className="text-sm whitespace-pre-wrap leading-relaxed text-slate-700 relative before:absolute before:inset-0 before:ml-2 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-200 before:to-transparent pl-6 border-l border-slate-200" style={getStyle('experience')}>
                {data.experience}
              </div>
            </div>
          )}
          ...
          {data.projects && (
            <div>
              ...
              <div className="text-sm whitespace-pre-wrap leading-relaxed text-slate-700 pl-6 border-l border-slate-200" style={getStyle('projects')}>
                {data.projects}
              </div>
            </div>
          )}
