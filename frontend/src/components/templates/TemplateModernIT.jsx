...
const TemplateModernIT = ({ data }) => {
  return (
    <div className="w-full bg-white text-slate-800 h-[842px] border border-slate-200 aspect-[1/1.414] font-sans flex flex-col overflow-hidden shadow-sm">
            ...
            <div className="mb-8">
              ...
              <div className="flex flex-col gap-2">
                {(Array.isArray(data.skills) ? data.skills : (data.skills ? data.skills.split(',') : [])).map((skill, index) => (
                  <span key={index} className="text-xs bg-slate-800 text-cyan-100 px-3 py-1.5 rounded border border-slate-700/50">
                    {skill}
                  </span>
                ))}
              </div>
        ...
        <div className="col-span-2 p-10 bg-slate-50 flex flex-col gap-10">
          ...
          {data.summary && (
            <div>
              ...
              <p className="text-sm leading-relaxed text-slate-600 border-l-2 border-cyan-200 pl-4">{data.summary}</p>
            </div>
          )}
